...
#
# Disable middle click and right click on Lenovo Thinkpad T14 touchpad.
...
if [ -n "${XINPUT_TOUCHPAD_ID}" ]; then
    xinput get-button-map ${XINPUT_TOUCHPAD_ID} >> /tmp/profile.d.log 2>&1
    xinput set-button-map ${XINPUT_TOUCHPAD_ID} 1 1 1 4 5 6 7 >> /tmp/profile.d.log 2>&1
fi
